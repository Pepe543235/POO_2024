...
CLASES .- es como un molde a traves del cual se puede instanciar un objeto dentro de las clases se definen los atributos 
(propiedades / caracteristicas) y los métodos (funciones o acciones)

OBJETOS O INSTANCIAS .- son parte de una clase los objetos o instacias pertenecen a una clase, es decir para interacturar 
con la clase o clases y hacer uso de los atributos y metodos es necesario crear un objeto o objetos.

METODO CONSTRUCTOR .- Este metodo se coloca dentro de la clase y se utiliza para dar un valor a los atributos del objeto el momento
de crearlo
"""
...
class Coches:
    ...
    #En python el encapsulamiento tambien se llama visibilidad y por lo general define como seran los atributos y metodos
    #es decir publicos o privados 
...
class Camiones(Coches):
  ...
  def getInfo(self):
        print(f"Marca: {self.getMarca()} {self.getColor()}, numeros de plazas: {self.getPlazas()} \nModelo: {self.getModelo()} con una velocidad de {self.getVelocidad()} Km/h y un potencia de {self.getCaballaje()} con {self.getEje()} ejes y una capacidad de carga de {self.getCapacidadCarga} metros cubicos")     
...
class Camionetas(Coches):
  ...
  def getInfo(self):
    print(f"Marca: {self.getMarca()} {self.getColor()}, numeros de plazas: {self.getPlazas()} \nModelo: {self.getModelo()} con una velocidad de {self.getVelocidad()} Km/h y un potencia de {self.getCaballaje()} con la tracción {self.getTraccion()} y es cerrada {self.getCerrada}")     
